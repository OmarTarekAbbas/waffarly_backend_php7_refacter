                    ...
                    <form action="{{url('setting/'.$setting->id)}}" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9 col-lg-10 controls">
                                <input type="text" name="key" id="key" value="{{$setting->key}}" placeholder="key"
                                    class="form-control" readonly required>
                            </div>
                        </div>

                        <div class="form-group">
                            @if($setting->type->title == "Image" || $setting->type->title == "Normal Editor" ||
                            ...
                            <div class="col-sm-9 col-lg-10 controls">
                                ...
                                @elseif($setting->type->title == "input_color")
                                <div class="col-sm-9 col-lg-10 controls">
                                    <input type="color" name="input_color" value="{{$setting->value}}" style="width: 122%;">
                                </div>
                                @else
                                ...
                                <select class="form-control" name="extensions[]" multiple>
                                    <option value="image" @foreach($selected_extensions as $extension)
                                        @if($extension=='image' ) selected @endif @endforeach>Images</option>
                                    <option value="audio" @foreach($selected_extensions as $extension)
                                        @if($extension=='audio' ) selected @endif @endforeach>Audios</option>
                                    <option value="video" @foreach($selected_extensions as $extension)
                                        @if($extension=='video' ) selected @endif @endforeach>Videos</option>
                                    <option value="text" @foreach($selected_extensions as $extension)
                                        @if($extension=='text' ) selected @endif @endforeach>Text</option>
                                    <option value="all" @foreach($selected_extensions as $extension)
                                        @if($extension=='all' ) selected @endif @endforeach>All Extensions</option>
                                </select>
                                @elseif($setting->value[0]=="<") <textarea name="value" name="value" placeholder="value"
                                    class="form-control col-md-12 ckeditor" required>{{$setting->value}}</textarea>
                                    @else
                                    <textarea name="value" name="value" placeholder="value"
                                        class="form-control col-md-12" required>{{$setting->value}}</textarea>
                                    @endif
                                    @endif
                            </div>
                        </div>
