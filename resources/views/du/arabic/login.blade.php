...
<section class="main-container-wrapper">
  ...
  <div class="c-popup l-modal__body">
      ...
      @if(Session::get('dataPhoneNo') != 0)
      <h3 class="c-popup__title">{{Session::get('dataPhoneNo')}}</h3>
      @endif

    <h3 class="c-popup__title">{{Session::get('Status')}}</h3>

    @if(Session::get('phone') )
     <a href="{{url(Session::get('OpID').'/directSubscribeAction?phone='.Session::get('phone'))}}">اشتراك</a>
    @endif

  @if(Session::get('dataPhoneNo') != 0)
<a href="{{url(Session::get('OpID').'/dataSubscribe')}}">اشترك الان</a>
  @endif
  </div>
</div>
    @endif
